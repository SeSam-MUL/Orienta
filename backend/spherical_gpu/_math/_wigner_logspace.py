...
def _csht_weights_half_pi_uncached(
    L: int,
    device: torch.device,
...
def _rsht_weights_half_pi_uncached(
    L: int,
    device: torch.device,
    precision: str = "double",
    batch_size: int = int(1e7),
...
# ---------------------------------------------------------------------------
# Memoization for the bandwidth-only half-pi weight tables.
#
# Both ``csht_weights_half_pi`` and ``rsht_weights_half_pi`` are pure functions
# of (L, device, precision) -- they are phase-INDEPENDENT but were being rebuilt
# once per phase (~1s each). Memoizing per (L, device, precision) lets the
# 2nd..Nth phase reuse the table. ``batch_size`` is deliberately NOT part of the
# key: it only controls internal chunking and does not affect the result.
#
# Aliasing safety: the call sites in ``sht.py`` register the returned tensor as
# an nn.Module buffer ``D`` (and a transposed *view* ``DT``) and only ever use
# it read-only (as the RHS of ``torch.mm``). Caching by reference would hand the
# SAME tensor object to multiple SHT instances built for the same key; a future
# ``module.to(...)`` / ``.float()`` could then mutate that shared buffer in
# place and silently corrupt the cache for every other live instance. To remove
# that risk we ``.clone()`` on a cache hit -- a clone is ~ms vs the ~1s build,
# so this is still a massive win, and a clone is value-identical (bit-identical)
# to the cached tensor.
# ---------------------------------------------------------------------------

_HALF_PI_CACHE_CAP = 4
_csht_half_pi_cache: "OrderedDict[Tuple[int, str, str], Tensor]" = OrderedDict()
_rsht_half_pi_cache: "OrderedDict[Tuple[int, str, str], Tensor]" = OrderedDict()


def clear_half_pi_caches() -> int:
    """Drop all cached half-pi Wigner weight tensors and return the count.

    These OrderedDicts hold *live* references to GPU tensors, so a bare
    ``torch.cuda.empty_cache()`` cannot reclaim them. The Release-GPU emergency
    button clears them for a full reset; the next SHT build simply recomputes
    (~1 s). Safe to call any time — a running build holds its own ``.clone()``
    of the weights, so clearing the cache never corrupts work in flight.
    """
    n = len(_csht_half_pi_cache) + len(_rsht_half_pi_cache)
    _csht_half_pi_cache.clear()
    _rsht_half_pi_cache.clear()
    return n
...
def csht_weights_half_pi(
    ...
    """Cached wrapper around :func:`_csht_weights_half_pi_uncached`.

    Memoized per ``(int(L), str(device), str(precision))``. ``batch_size`` is
    not part of the key. Returns a value-identical clone of the cached tensor.
    See :func:`_csht_weights_half_pi_uncached` for the math and return shape.
    """
    key = (int(L), str(device), str(precision))
    cached = _csht_half_pi_cache.get(key)
    if cached is not None:
        _csht_half_pi_cache.move_to_end(key)
        return cached.clone()
    wig = _csht_weights_half_pi_uncached(L, device, precision, batch_size)
    _csht_half_pi_cache[key] = wig
    while len(_csht_half_pi_cache) > _HALF_PI_CACHE_CAP:
        _csht_half_pi_cache.popitem(last=False)
    return wig.clone()
...
def rsht_weights_half_pi(
    ...
    """Cached wrapper around :func:`_rsht_weights_half_pi_uncached`.

    Memoized per ``(int(L), str(device), str(precision))``. ``batch_size`` is
    not part of the key. Returns a value-identical clone of the cached tensor.
    See :func:`_rsht_weights_half_pi_uncached` for the math and return shape.
    """
    key = (int(L), str(device), str(precision))
    cached = _rsht_half_pi_cache.get(key)
    if cached is not None:
        _rsht_half_pi_cache.move_to_end(key)
        return cached.clone()
    wig = _rsht_weights_half_pi_uncached(L, device, precision, batch_size)
    _rsht_half_pi_cache[key] = wig
    while len(_rsht_half_pi_cache) > _HALF_PI_CACHE_CAP:
        _rsht_half_pi_cache.popitem(last=False)
    return wig.clone()