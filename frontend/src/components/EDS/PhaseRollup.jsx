/**
 * "11 regions → 4 phases", with the phases themselves.
 *
 * The region list names a phase once per STRUCTURE, so a phase that covers
 * four regions is written four times. That reads as a bug — the user's
...
/**
 * Fold the region list down to the phases it carries.
 *
 ...
 */
export function rollUpPhases(regions, allPhases) {
  ...
  // The colour has to come from the phase list, not from a region: the
  // region palette is a different scheme on purpose.
...
/** How many regions carry no phase yet. */
export function countUnnamed(regions) {
  return (regions || []).filter((s) => s.phase_index == null || s.phase_index < 0).length;
}

export default function PhaseRollup({ regions, allPhases, onShowPhases, totalPixels }) {
  const { t } = useTranslation('eds');
  const phases = useMemo(() => rollUpPhases(regions, allPhases),
    [regions, allPhases]);
  const unnamed = countUnnamed(regions);
        ...
        {phases.map((p) => (
          ...
          >
            ...
            <span style={{ color: C.textSecondary, flexShrink: 0,
              ...
              {t('rollup.fromRegions', { count: p.n_regions })}
            </span>
            {totalPixels > 0 && (
