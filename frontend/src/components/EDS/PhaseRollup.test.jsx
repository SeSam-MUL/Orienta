...
/**
 * "11 structures → 4 phases".
 *
 * The user read the repeated phase names in the structure list as a fault:
 * "die tauchen dann ca 100 mal in der Liste auf". They are not — a phase
 * covering four structures is written four times because the list is per
 * structure. This box counts each phase once and points at the view where
...
const STRUCTURES = [
  { structure_id: 0, n_pixels: 3000, phase_index: 0, cif_filename: 'Al.cif' },
  { structure_id: 1, n_pixels: 2000, phase_index: 0, cif_filename: 'Al.cif' },
  { structure_id: 2, n_pixels: 500, phase_index: 1, cif_filename: 'Si.cif' },
  { structure_id: 3, n_pixels: 100, phase_index: -1, cif_filename: null },
];
...
describe('rollUpPhases', () => {
  it('counts a phase once however many structures carry it', () => {
    const rolled = rollUpPhases(STRUCTURES, ALL_PHASES);
    expect(rolled).toHaveLength(2);
    const al = rolled.find((p) => p.cif_filename === 'Al.cif');
    expect(al.n_structures).toBe(2);
    expect(al.n_pixels).toBe(5000);
  });

  it('leaves out phases no structure carries', () => {
    const rolled = rollUpPhases(STRUCTURES, ALL_PHASES);
    expect(rolled.map((p) => p.cif_filename)).not.toContain('Cu.cif');
  });

  it('ignores structures that have no phase yet', () => {
    const rolled = rollUpPhases(STRUCTURES, ALL_PHASES);
    expect(rolled.reduce((n, p) => n + p.n_structures, 0)).toBe(3);
  });

  it('takes the colour from the phase list, not from a structure', () => {
    // The structure palette is a different scheme on purpose; borrowing it
  ...
  it('keys on the phase index, so two entries sharing a name stay apart', () => {
    const structures = [
      { structure_id: 0, n_pixels: 10, phase_index: 0, cif_filename: 'Al.cif' },
      { structure_id: 1, n_pixels: 10, phase_index: 1, cif_filename: 'Al.cif' },
    ];
    const phases = [
      { phase_index: 0, cif_filename: 'Al.cif', color: '#111111' },
      { phase_index: 1, cif_filename: 'Al.cif', color: '#222222' },
    ];
    expect(rollUpPhases(structures, phases)).toHaveLength(2);
  });
...
describe('countUnnamed', () => {
  it('counts the structures with no phase', () => {
    expect(countUnnamed(STRUCTURES)).toBe(1);
  });

  it('treats a missing phase_index as unnamed rather than as phase 0', () => {
    expect(countUnnamed([{ structure_id: 0, n_pixels: 1 }])).toBe(1);
  });
});

describe('the roll-up box', () => {
  it('states the collapse in one line', () => {
    render(<PhaseRollup structures={STRUCTURES} allPhases={ALL_PHASES} />);
    expect(screen.getByText('rollup.headline:{"structures":4,"phases":2}'))
      .toBeTruthy();
  });

  it('warns while structures are still unnamed', () => {
    render(<PhaseRollup structures={STRUCTURES} allPhases={ALL_PHASES} />);
    expect(screen.getByText('rollup.unnamed:{"count":1}')).toBeTruthy();
  });

  it('says nothing about unnamed structures once there are none', () => {
    const named = STRUCTURES.filter((s) => s.phase_index >= 0);
    render(<PhaseRollup structures={named} allPhases={ALL_PHASES} />);
    expect(screen.queryByText(/rollup\.unnamed/)).toBeNull();
  });

  it('shows how many structures each phase came from', () => {
    render(<PhaseRollup structures={STRUCTURES} allPhases={ALL_PHASES} />);
    expect(screen.getByText('rollup.fromStructures:{"count":2}')).toBeTruthy();
  });
  ...
  it('renders nothing without structures', () => {
    const { container } = render(
      <PhaseRollup structures={[]} allPhases={ALL_PHASES} />);
