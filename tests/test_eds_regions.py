"""Regions: group the map by chemistry first, name it afterwards.
...
* ``phase_grid`` stays authoritative. It has five consumers outside the store
  ...
  xmap), so a region assignment must WRITE INTO it, not replace it.
* Hand edits win over regions. A boundary move must never silently undo a
...
def _store(region_grid=None, region_phase=None, features=None):
    """A 4x4 map whose left half is region 0 and right half region 1."""
    st = PhaseMapStore()
    if region_grid is None:
        region_grid = np.zeros((4, 4), dtype=np.int32)
        region_grid[:, 2:] = 1
    if region_phase is None:
        region_phase = [-1] * (int(region_grid.max()) + 1)
    if features is None:
        # One feature: the column index, so a split cuts left/right predictably.
        features = np.repeat(
            np.arange(region_grid.shape[1], dtype=float),
            1).reshape(1, -1).repeat(region_grid.shape[0], axis=0).reshape(-1, 1)
    st.set_classification(
        phase_grid=np.full(region_grid.shape, -1, dtype=np.int32),
        score_grid=np.zeros(region_grid.shape, dtype=np.float32),
...
def test_regions_live_beside_phase_grid_not_instead_of_it():
    st = _store()
    state = st.get_state()
    assert state.phase_grid.shape == state.region_grid.shape
    assert state.n_regions == 2


def test_naming_a_region_paints_every_pixel_of_it():
    st = _store()
    n = st.assign_region_phase(1, 0)          # right half -> Al
    assert n == 8
    grid = st.get_state().phase_grid
    assert (grid[:, 2:] == 0).all()
    assert (grid[:, :2] == -1).all(), "the other region must not move"


def test_naming_does_not_lock_the_pixels():
    """The region IS the record of the decision, so a later boundary
    change keeps following it. Locking would freeze the pixels in place."""
    st = _store()
    st.assign_region_phase(0, 1)
    lm = st.get_state().locked_mask
    assert lm is None or not lm.any()


def test_naming_an_unknown_region_raises():
    st = _store()
    with pytest.raises(ValueError):
        st.assign_region_phase(9, 0)


def test_naming_with_an_unknown_phase_raises():
    st = _store()
    with pytest.raises(ValueError):
        st.assign_region_phase(0, 99)


def test_naming_without_regions_says_so_rather_than_guessing():
    st = PhaseMapStore()
    st.set_classification(
    ...
    with pytest.raises(RuntimeError):
        st.assign_region_phase(0, 0)


# --- merge -------------------------------------------------------------------

def test_merge_folds_one_region_into_another():
    st = _store()
    st.assign_region_phase(0, 0)
    moved = st.merge_regions(0, 1)
    assert moved == 8
    state = st.get_state()
    assert state.n_regions == 1
    assert (state.region_grid == 0).all()
    assert (state.phase_grid == 0).all(), "the merged pixels take the kept name"


def test_merge_keeps_the_id_list_dense():
    """Ids are positions; a hole would misname every region after it."""
    grid = np.array([[0, 1], [2, 2]], dtype=np.int32)
    st = _store(grid, [-1, 0, 1])
    st.merge_regions(0, 1)
    state = st.get_state()
    assert state.n_regions == 2
    assert set(np.unique(state.region_grid)) == {0, 1}
    assert state.region_phase == [-1, 1], "region 2's name must follow it down"


def test_merge_onto_itself_is_a_no_op():
    st = _store()
    assert st.merge_regions(0, 0) == 0
    assert st.get_state().n_regions == 2


def test_merge_is_undoable():
    st = _store()
    before = st.get_state().region_grid.copy()
    st.merge_regions(0, 1)
    st.undo()
    assert (st.get_state().region_grid == before).all()
    assert st.get_state().n_regions == 2


def test_merge_rejects_an_unknown_region():
    st = _store()
    with pytest.raises(ValueError):
        st.merge_regions(0, 7)


# --- split -------------------------------------------------------------------

def test_split_cuts_one_region_and_leaves_the_rest_alone():
    """Raising the global cluster count would re-cut every region; that is
    why splitting is local."""
    st = _store()
    other_before = (st.get_state().region_grid == 1).copy()
    n = st.split_region(0, 2)
    assert n == 2
    state = st.get_state()
    assert state.n_regions == 3
    assert ((state.region_grid == 1) == other_before).all()


def test_split_parts_inherit_the_original_name():
    st = _store()
    st.assign_region_phase(0, 2)
    st.split_region(0, 2)
    state = st.get_state()
    assert state.region_phase[0] == 2
    assert state.region_phase[-1] == 2


def test_split_of_a_region_smaller_than_the_parts_is_a_no_op():
    grid = np.array([[0, 1], [1, 1]], dtype=np.int32)
    st = _store(grid, [-1, -1])
    assert st.split_region(0, 3) == 0


def test_split_without_the_composition_fails_loud():
    """After a backend restart the sidecar has the regions but not the
    composition. Guessing would split on the wrong data."""
    st = _store()
    st._region_features = None
...
def test_grow_pushes_the_boundary_out():
    st = _store()
    before = int((st.get_state().region_grid == 0).sum())
    st.grow_region(0, 1)
    assert int((st.get_state().region_grid == 0).sum()) > before
...
def test_shrink_hands_the_pixels_to_a_neighbour_not_to_nothing():
    ...
    st = _store()
    st.grow_region(0, -1)
    grid = st.get_state().region_grid
    assert (grid >= 0).all(), "no pixel may be left without a region"


def test_grow_by_zero_changes_nothing():
    st = _store()
    assert st.grow_region(0, 0) == 0
...
def test_snap_moves_a_boundary_onto_the_real_chemistry_edge():
    ...
    st = _store(grid, [-1, -1], feats)
    st.snap_region_edges(strength=2.0)
    out = st.get_state().region_grid
